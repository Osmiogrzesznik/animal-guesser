...
class GuessGame():
    def __init__(self):
        ...
        # Dictionary that holds question:answer value pairs for current THING
    ...
    def largest(self, st):
        ...
        t = list(zip([len(self.db[x][YES])+len(self.db[x][NO])
                      for x in st], [x for x in st]))
        return max(t)[1]
    ...
    def askquestion(self):
        ...
        print(_('Is Your %s %s? (y/n)') % (THING, c_question), end=' ')
        ca = self.ask()
        if ca is None:
            return
    ...
    def guess(self):
        ...
        print(_('Is Your %s %s? (y/n) ') % (THING, actual))
        ca = self.ask()
        if ca == YES:
            print(_('I Won, hahaha'))
            self.learn_new(actual)
            self.win = True
        if ca == NO:
            print(_('It can\'t be, nooooo'))
            self.tries -= 1
            self.points.pop(actual, None)
            if actual in self.answers:
                self.answers.remove(actual)
    ...
    def play(self):
        while True:
            ...
            if self.tries > 0 and len(self.answers) > 0 and len(self.questions) > 0:
                continue

            # If there are no answers left, surrender. Ask for name of the new_thing, and suitable question.
            else:
                print(_('I give up. What %s was it ?:') % (THING))
                new_thing = input('').lower()
                print(_("\nPerfect.\nCould you ask a 'yes or no' question to differentiate your %s? (The answer must be yes)") % (THING))
                print(_("Complete the following sentence: 'Your %s ......?'") % (THING))
                while True:
                    print(_("Your %s...") % (THING), end=' ')
                    ...
                    if new_question in self.questions or new_question in self.current_answers:
                        ...
                        print(_('I already know that question, try another one.'))
...
if __name__ == '__main__':
    print(_('Let\'s play a game, think of a %s, whenever you want, hit enter.') % (THING))
    input('')
    while True:
        game = GuessGame()
        game.play()
        print(_('Would you like to play again?'))
        ca = game.ask()
        if ca == YES:
            continue
